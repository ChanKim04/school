...
class CalOS:
    ...
    def timer_isr(self):
        ...
        elif not self._ready_q:
            self.reset_timer()
            self._cpu._registers['reg0'] = 0
            self._cpu._registers['reg1'] = 0
            self._cpu._registers['reg2'] = 0
            self._cpu._registers['pc'] = 0
            return
        self.context_switch()
        self.reset_timer()
    ...
    def context_switch(self):
        ...
        CalOS.current_proc.set_registers(self._cpu.get_registers())
        self._cpu.set_registers(newpcb.get_registers())
        self._ready_q.append(CalOS.current_proc)
        newpcb.set_state(PCB.RUNNING)
        CalOS.current_proc = newpcb
