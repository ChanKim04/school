...
class CPU:
    ...
    def _get_value_at(self, addr):
        ...
        or hex.'''
        addr = eval(addr[1:])
        return self._mmu.get_value(addr)                  # self._ram[addr]
    ...
    def handle_mov(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.set_value(self._registers[dst[1:]], srcval)            # self._ram[self._registers[dst[1:]]] = srcval 
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.set_value(eval(dst), srcval)                                       # self._ram[eval(dst)] = srcval  
    ...
    def handle_add(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            self._mmu.set_value(eval(dst), (self._mmu.get_value(eval(dst)) + srcval))                        # self._ram[eval(dst)] += srcval
