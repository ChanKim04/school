...
@date: 2/6/18
@changed | added: added help message for batch mode R <addr>
                  added a method _run_batch(self, addr) 
'''
...
class Monitor:
        ...
        while True:
            try:
                ...
                if instr == '?':
                    ...
                    # added batch mode message
                    print("R <addr>: execute program starting at addr by Batch mode")
    ...
    # added batch mode run which include isBatch parameter as True for indicating batch mode
    def _run_batch(self, addr):
        # creates a new thread, passing in ram, the os, and the
        # starting address
        self._cpu = CPU(self._ram, calos.CalOS(), addr, self._debug, 0, True)
        self._cpu.start()		# call run()
        self._cpu.join()		# wait for it to end
