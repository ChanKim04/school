...
class CPU(threading.Thread):
    def __init__(self, ram, os, startAddr, debug, num=0, isBatch=False): # added a boolean parameter isBatch
        ...
        # _isBatch indicate batch mode
        # _startAddr shows current addr
        # _programs is a list to store programs for batch mode
        self._isBatch = isBatch
        self._startAddr = startAddr
        self._programs = []
        # if batch mode is on, it stores its addr's value to the list unless the value is 0.
        if (isBatch == True):
            addr = self._registers['pc']
            val = self._ram[addr]
            while ( val != 0):
                self._programs.append(val)
                addr += 1
                val = self._ram[addr]
    ...
    def run(self):
        ...
        # False means it does not run batch mode
        if (self._isBatch == False):
            self._run_program()
        # True means it runs the programs with batch mode
        elif (self._isBatch == True):
            # the value at the memory location is valid, run programs with batch mode, otherwise print the message.
            try:
                # run programs from the list _programs 
                for i in self._programs:
                    # clear the register before trying to run each program
                ...
                print("Invalid value at the location")
